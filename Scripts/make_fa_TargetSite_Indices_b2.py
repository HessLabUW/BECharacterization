###############################################################################
# Modified by Gaelen Hess on 3/29/2023
# adapted from makeIndices.py in casTLE
################################################################################
...
working_dir="/project8/HessLabCHGPM/Users/ghess3/BEChar/FinalData/FastqProcessing/"
index_dir=os.path.join(working_dir, "Indices")
...
index_file = os.path.join(index_dir, 'screen_type_index.txt')
index = []
...
##############################################################################
# Convert oligo file into bowtie2-compatible fasta
...
fasta_location = os.path.join(index_dir, 'temp_fasta.fna')
...
##############################################################################
# Call bowtie2-build to build new index

try:
    subprocess.check_call(args.bowtie + ' ' + fasta_location + ' ' + 
                                            os.path.join(index_dir, args.full_name),
                                                                    shell=True)
except:
    sys.exit()
...
index_name = os.path.join(index_dir, args.full_name)
