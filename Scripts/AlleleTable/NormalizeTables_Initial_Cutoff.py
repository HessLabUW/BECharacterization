...
# This script is used for files generated by UMI_Filter_ProcComp as it parses the fields differently than regular UMI_Filter.py
################################################################################
...
# Only needed in initial filtering comparing filtering methods
# This section process the sample name to break it into how it was edited (treatment) and which filtering method was used (process_group) 
master_table_cntfilt['treatment']=master_table_cntfilt.apply(lambda x: x['sample'].split("-")[-1], axis=1)
master_table_cntfilt['process_group']=master_table_cntfilt.apply(lambda x: "-".join(x['sample'].split("-")[:-1]), axis=1)
